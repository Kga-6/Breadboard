...
export async function GET(req: NextRequest) {
  ...
  const passageId = pathParts[pathParts.indexOf("passages") + 1];

  if (!bibleId || !passageId) {
    return NextResponse.json({ error: "Missing bibleId or passageId" }, { status: 400 });
  }

  try {
    // 1. Fetch from API
   const res = await fetch(`${BASE_URL}/bibles/${bibleId}/passages/${passageId}`, {
